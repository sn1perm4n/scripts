...
function Remove-EmptyFoldersStrict {
	param (
		[Parameter(Mandatory=$true)]
		[string]$Path
	)

	# List of critical/protected folders to skip. Do not allow scanning of protected folders. Immediately stop execution if the user selects a critical/protected folder. Change <PROFILE> to match your system.
	$ProtectedFolders = @(
	...
	if ($ProtectedFolders | Where-Object { $folderToScan -ieq $_ }) {
		Write-Host "WARNING: The folder '$folderToScan' is protected and cannot be scanned." -ForegroundColor Yellow
		return
	}
	...
	if ($deletedFolders.Count -gt 0) {
		...
		Write-Host "`nNo folders were deleted."
	}
}

# Prompt user for the folder to scan
$folderToScan = Read-Host "Enter the full path of the folder you want to scan"

# Run the function
Remove-EmptyFoldersStrict -Path $folderToScan
