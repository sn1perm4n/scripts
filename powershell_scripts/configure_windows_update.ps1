...
try {
	...
	if (-not (Get-ItemProperty -Path $regPath -Name "IsContinuousInnovationOptedIn" -ErrorAction SilentlyContinue)) {
		...
		Set-ItemProperty -Path $regPath -Name "IsContinuousInnovationOptedIn" -Value 1 -Force -ErrorAction Stop | Out-Null
	}
	...
	if (-not (Get-ItemProperty -Path $regPath -Name "AllowMUUpdateService" -ErrorAction SilentlyContinue)) {
		...
		Set-ItemProperty -Path $regPath -Name "AllowMUUpdateService" -Value 1 -Force -ErrorAction Stop | Out-Null
	}
