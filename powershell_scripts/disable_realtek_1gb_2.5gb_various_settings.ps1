# This script disables various settings on Realtek 1Gb and 2.5Gb Ethernet cards that negatively impact performance (especially for gaming). The name of your Ethernet adapter may differ (i.e. on one of my systems it's "Local Area Connection") so confirm first with the following PowerShell command (if you don't do this you'll see a bunch of red errors): Get-NetAdapter
# I recommend that you confirm your Realtek NIC driver is up-to-date via your motherboard manufacturer's support website (or better yet your motherboard manufacturer's software, i.e. MSI Center, Armoury Crate, etc.). You will need to disable and re-enable your network adapter (or just restart your PC) for the below changes to take effect.
# I also recommend re-running this script every time you update your Realtek Ethernet driver as there's a chance the settings could be reset to their default values.
# Lastly, it would be a good idea to confirm the below settings exist for your NIC by running the following PowerShell command and comparing the output to this script: Get-NetAdapterAdvancedProperty -Name "Ethernet" | Sort-Object "DisplayName"
